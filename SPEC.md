...
- Cached **per language** as `data/glossary/glossary-<lang>.json` beside the
  ...
  a community language pack (#163); with no pack it never produces a file and
  therefore never receives official-term hints — the editor, local-AI prompt, and
  batch export all see no glossary for it.
...
| --------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `token-missing`       | Error    | A **protected token** present in the source is missing (or under-represented) in the target. This includes structural `#` and repeated `^` markers (`^^` counts as two); apostrophes inside words such as `don't` are prose, not tokens. This will break the mod at runtime or corrupt its formatting. **Exemptions:** `\n` (layout) and paired `'` quote delimiters (punctuation) are not syntax — their count differences are covered by `newline-mismatch` / `quote-mismatch` below, never by this error. |
| `token-added`         | Error    | The target contains more occurrences of a **protected token** than the source. Protected-token counts must match exactly; `\n` and paired `'` quote delimiters remain exempt.                                                                                                                                                                                                                                                                                                                                |
...
| `quote-mismatch`      | Warning  | The paired `'` quote-delimiter count differs between source and target. Punctuation, not runtime syntax in SMAPI i18n: adding, removing, or restyling quotes (e.g. backticks or `„…“` in the translation) never breaks a mod — see the scope note below.                                                                                                                                                                                                                                                     |
...
> **Scope note (2026-06-14):** the same reasoning applies to the paired `'` quote delimiter. It was a hard `token-missing` / `token-added` token, so a translation that legitimately restyled quotes — e.g. a source using backticks (`` `Default` ``) translated as `'Standard'` — was flagged as an **error** and blocked export, even though quotes are punctuation and never break a mod at runtime (unlike `{{...}}`, `$b`, `#`, `^`, `@`). Fixed by exempting `'` from the token error rules, mirroring `\n`, and reporting count differences via the new `quote-mismatch` **warning**. Token _extraction_ (the editor chips) is unchanged.
...
| Structural separators   | `#`, paired quote delimiters in `'test'`        |
| Single-character        | `@` (player name), `^` / `\n` (line break)      |

Tokens are compared as **multisets** (counts matter, order does not): every token in the source must appear in the target the same number of times. This catches a dropped second `$b` or one missing `^` from `^^`, not just a missing distinct token. Word-internal apostrophes (`don't`, `farmer's`) remain normal prose and are not extracted.

**Soft tokens.** `\n` and paired `'` quote delimiters are still _extracted_ (shown as editor chips) but are **exempt from the blocking `token-missing` / `token-added` errors** — a count difference surfaces as the `newline-mismatch` / `quote-mismatch` **warning** instead, because neither breaks a mod at runtime. All other recognized kinds are hard tokens.
...
| Mods can register additional custom languages via `Data/AdditionalLanguages` (SV 1.6). SMAPI then loads `i18n/<LanguageCode>.json` for that locale — e.g. Thai uses `th`. | [SMAPI Wiki — Custom languages](https://stardewvalleywiki.com/Modding:Modder_Guide/APIs/Translation)            |
