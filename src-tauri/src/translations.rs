...
pub fn language_root(config_dir: &Path, target_lang: &str) -> Result<PathBuf, String> {
    if config_dir.join("community-work-context.json").exists() {
        // Until personal state is migrated, do not reuse it in another installation.
        crate::community_library::context(config_dir)?;
    }
