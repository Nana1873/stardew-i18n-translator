...
4. Strings with **error-level** issues (`token-missing`) are **skipped** individually (not a hard block on the whole file); all other strings export normally, and the summary reports what was skipped and why. Untranslated and not-translatable strings are omitted (SMAPI falls back to `default.json`). See [SPEC.md §10 / §17 M3](../../SPEC.md). ✅ — _(`json-invalid` cannot occur from a Rust `String`, which is always valid UTF-8, so that error is moot at export time.)_
