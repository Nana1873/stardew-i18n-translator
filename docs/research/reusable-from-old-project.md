...
The reusable logic spans **both Rust and TypeScript**, and the old project was Tauri 2. This is why [ADR 0001](../adr/0001-tech-stack-decision.md) chose **Tauri**: it reuses the Rust hot path _and_ the TS UI logic _and_ the fixtures, while giving the best performance/memory on very large mods. Electron would reuse only the TS; Python+PySide6 would discard all of it.
