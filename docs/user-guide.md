...
Translator instead of sending original archives to Vortex. Valid imported values are marked **Done**; existing personal values are kept. Use the **Workspace** tab for manual or AI-assisted editing. Already imported Nexus files
are remembered separately from translation completeness, so an incomplete archive
does not require downloading again. Search progress shows checked Nexus IDs, then a separate loading phase while file versions are fetched. Import progress shows the current file and processed-file count. The result separates imported values, archives needing attention, and text skipped by token checks; individual downloads have no estimated byte percentage. **Stop after current** finishes the active file before stopping the batch. Successfully imported exact archives are hidden by default, even when some strings remain missing or were skipped by token checks. Enable **Show already imported** to inspect or re-import them, including at 100% working coverage. Explicit searches still check known imported mods for newer eligible files; new files and unimported relevant supplements remain visible even when all working text is present. Failed, unresolved and unknown attempts remain visible. Older import receipts without a complete-attempt record require a deliberate re-import to establish that status; they are never automatically downloaded again. Nexus coverage includes verified installed components associated with the same source page, even across separate physical packages. Workspace progress follows the selected component or package. Saved import identities from original-mod translation matches also retain package siblings whose manifests have no usable Nexus ID. Related add-on receipts and components with a different explicit Nexus ID do not expand the original mod totals through saved receipts. Import status stays compact in the results; coverage and component diagnostics are available under **Details**. Continue editing through the existing Workspace. **Re-import translation** under **Details** explicitly downloads the selected, already imported archive again; existing saved text is kept. It never runs automatically. **Recheck import** retries unmatched files or newly discovered translation units within an imported component; existing saved edits are kept. Missing text in another component does not by itself mean that importing the same archive again can fill it.
...
Optional downloads on other original-translation pages can supplement installed components when their full component name matches unambiguously. The selected main translation stays unchanged. Single-component mods do not receive automatic supplements, and components already mapped by the main import are excluded even when some strings were skipped. These additions are included in **Download & import all**; each has a collapsed component details section with its Nexus files link for manual download and a retry action when an import fails or leaves unmatched files. Uninstalled components and ambiguous names are not offered. Successfully imported exact supplemental files are hidden by default; newer eligible supplemental files can appear even at complete working coverage. Native import checks still apply, and supplemental imports are limited to the named component.
...
memory and no mod assets are installed. The **Workspace** tab lets you inspect the
