...
Optional downloads on other original-translation pages can supplement installed components with missing text when their full component name matches unambiguously. The selected main translation stays unchanged. Single-component mods do not receive automatic supplements, and components already mapped by the main import are excluded even when some strings were skipped. These additions are included in **Download & import all**; each has a collapsed component details section with its Nexus files link for manual download and a retry action when an import fails or leaves unmatched files. Uninstalled components, ambiguous names, completed text and previously imported files are not offered. Native import checks still apply, and supplemental imports are limited to the named component.
